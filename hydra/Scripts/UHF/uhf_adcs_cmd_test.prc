...
while beacon_cmd_succ_count < $cmdCnt
    cmd_noop
    cmd_set_pkt_rate apid 43 rate 3 stream UHF
    set cmdTry = cmdTry + 1
    wait 3500
endwhile
set cmdSucceed = cmdSucceed + 1

; Route same packets to SD card
set cmdCnt = beacon_cmd_succ_count + 1
while beacon_cmd_succ_count < $cmdCnt
    cmd_noop
...
echo Test tries = $cmdTry, Successes = $cmdSucceed