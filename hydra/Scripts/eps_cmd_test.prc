...
;Press GO to test EPS commands (power cycle, heaters)
pause
...
set cmdCnt = beacon_cmd_succ_count + 1
while beacon_cmd_succ_count < $cmdCnt
	cmd_set_pkt_rate apid 2 rate 1 stream 2
	set cmdTry = cmdTry + 1
	wait 3500
endwhile
set cmdSucceed = cmdSucceed + 1
...
BATTERY:
call Scripts\battHtr_test

SD_CARDS:
call Scripts\eps_sd_card_power_test
